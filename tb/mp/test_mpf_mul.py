"""mpf_mul: RTL vs golden model (bus-exact, including ovf/unf flags)."""
...
from common.mpf_stim import random_mpf
...
async def _run(dut, pairs: list[tuple[mf.MPF, mf.MPF]]) -> None:
    ...
    for i, ((x, y), (rw, rovf, runf)) in enumerate(zip(pairs, got, strict=True)):
        exp, eovf, eunf = mf.mpf_mul(x, y, FMT)
        ew = mf.pack(exp, FMT)
...
async def directed(dut) -> None:
    ...
    maxm = mf.MPF(0, 0, (1 << w) - 1)
    pairs = [
        (one, one),
        (maxm, maxm),  # forces round-up overflow path
        (one, mf.zero(1)),
        (mf.special(1), one),
        (mf.MPF(0, FMT.emax, 1 << (w - 1)), mf.MPF(0, FMT.emax, 1 << (w - 1))),  # ovf
        (mf.MPF(0, FMT.emin + 1, 1 << (w - 1)), mf.MPF(0, FMT.emin + 1, 1 << (w - 1))),  # unf
    ]
